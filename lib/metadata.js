...
function getFetchPromise(type, params, callback) {
    ...
    if (fetchPromises[type][key]) {
        ...
        promise.finally(() => { 
            console.log('deleting cached promise with key: ' , key);
...
async function fetchInfo(params) {
    ...
    try {
        let info = {};
        if (params.type === 'song') {
            info = await getSongInfo(params);
        }
        else if (params.type === 'album') {
            info = await getAlbumInfo(params);
        }
        else if (params.type === 'artist') {
            info = await getArtistInfo(params);
        }
