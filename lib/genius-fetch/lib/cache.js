...
class Cache {
  ...
  setTTL(type, ttl) {
    this._debug(`[genius-fetch-cache] Set TTL=${ttl}`);
    if (this._ttl[type] != ttl) {
      const keys = this.getKeys(type);
      for (const key of keys) {
        this._cache.ttl(key, ttl);
      }
      this._debug(`[genius-fetch-cache] Updated TTL of ${keys.length} entries to ${ttl}s`);
    }
    this._ttl[type] = ttl;
  }
  ...
  setMaxEntries(type, maxEntries) {
    ...
    this._debug(`[genius-fetch-cache] Set max entries to ${maxEntries}`);
  }
  ...
  put(type, key, value) {
    ...
    this._debug(`[genius-fetch-cache] Saving cache entry; key='${key}'`);
  ...
  clear(type) {
    ...
    this._debug(`[genius-fetch-cache] Cache cleared`);
  }
  ...
  async getOrSet(type, key, promiseCallback) {
    ...
    if (cachedValue !== undefined) {
      this._debug(`[genius-fetch-cache] Returning cached value for key '${key}'`);
      return cachedValue;
    }
